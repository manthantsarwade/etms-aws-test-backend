...
public class TaskServiceImpl implements TaskService {
    ...
    // Original Windows path commented out
    // private final String uploadDir = "C:\\Users\\intel\\Desktop\\front\\files";
    // private final Path root = Paths.get("C:\\Users\\intel\\Desktop\\front\\files");
    
    // Use environment variable or default to Docker-friendly path
    private final String uploadDir = System.getenv("UPLOAD_DIR") != null 
            ? System.getenv("UPLOAD_DIR") 
            : "/app/files"; // Default upload directory inside Docker
    ...
    public ApiResponse updateTask(Long taskId, MultipartFile file) throws IOException {
        ...
        if (file != null && !file.isEmpty()) {
            ...
            // UPDATED: Use resolve to properly combine the absolute uploadDir with the subdirectory
            Path uploadPath = Paths.get(uploadDir).resolve(subDirectory);
            
            // Create directories safely (createDirectories will not throw an error if the directory already exists)
    ...
    public ApiResponse approveRequest(Long id) {
        Task task = taskRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("invalid id not present"));
    ...
    public Resource load(Long taskId) {
        ...
        try {
            // UPDATED: Use the absolute file location directly
            Path file = Paths.get(task.getFile().getFileLocation());
